...
@router.get("/eventos-rh", response_model=list[EventoMarketing])
def read_eventos(session:SessionDep) -> list[EventoMarketing]:
    eventos  = session.exec(select(EventoMarketing)).all()
    return eventos

@router.get("/eventos-rh/{evento_id}", response_model = EventoMarketing)
...
@router.post("/eventos-rh", response_model=EventoMarketing)
def create_evento(evento:EventoMarketingCreate, session:SessionDep) -> EventoMarketing:
    new_evento = EventoMarketing(data_inicio = evento.data_inicio, data_fim = evento.data_fim, evento = evento.evento, descricao = evento.descricao, status = evento.status)
    session.add(new_evento)
    session.commit()
    session.refresh(new_evento)
    return new_evento

@router.put("/eventos-rh/{evento_id}", response_model= EventoMarketing)
...
@router.delete("/eventos-rh/{evento_id}", response_model=EventoMarketing)
