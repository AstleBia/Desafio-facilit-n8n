...
@app.get("/eventos-rh", response_model=list[EventoRh])
def read_eventos(session:SessionDep) -> list[EventoRh]:
    eventos  = session.exec(select(EventoRh)).all()
    return eventos

@app.get("/eventos-rh/{evento_id}", reponse_model = EventoRh)
...
@app.post("/eventos-rh", response_model=EventoRh)
def create_evento(evento:EventoRhCreate, session:SessionDep) -> EventoRh:
    new_evento = EventoRh(data_inicio = evento.data_inicio, data_fim = evento.data_fim, evento = evento.evento, descricao = evento.descricao, alcance = evento.alcance, status = evento.status)
    session.add(new_evento)
    session.commit()
    session.refresh(new_evento)
    return new_evento

@app.put("/eventos-rh/{evento_id}", response_model= EventoRh)
...
@app.delete("/eventos-rh/{evento_id}", response_model=EventoRh)
